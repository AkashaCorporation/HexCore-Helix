...
use helix_core::ir::hir::{
    HirBinOp, HirCase, HirExpr, HirFunction, HirModule, HirStmt, HirStorage, HirType, HirUnaryOp,
    HirVar, HirVarId, Span,
};
...
fn arb_hir_type() -> impl Strategy<Value = HirType> {
    arb_hir_type_leaf().prop_flat_map(|inner| {
        prop_oneof![Just(inner.clone()), Just(HirType::Pointer(Box::new(inner))),]
    })
}
...
fn arb_hir_var() -> impl Strategy<Value = HirVar> {
    (
        ...
        .prop_map(|(id, name, ty, storage, span)| HirVar {
            id,
            name,
            ty,
            storage,
...
fn arb_hir_expr_leaf() -> impl Strategy<Value = HirExpr> {
    prop_oneof![
        // IntLit
        (any::<i64>(), arb_hir_type(), arb_span()).prop_map(|(value, ty, span)| HirExpr::IntLit {
            value,
            ty,
            span
        }),
    ...
    arb_hir_expr_leaf().prop_recursive(
        ...
        |inner| {
            prop_oneof![
                ...
                // Unary
                (arb_unary_op(), inner.clone(), arb_hir_type(), arb_span()).prop_map(
                    |(op, operand, ty, span)| HirExpr::Unary {
                        op,
                ...
                // Cast
                (inner.clone(), arb_hir_type(), arb_span()).prop_map(|(expr, to_ty, span)| {
                    HirExpr::Cast {
                        expr: Box::new(expr),
                        to_ty,
                        span,
                ...
                // Subscript
                (inner.clone(), inner.clone(), arb_hir_type(), arb_span()).prop_map(
                    |(base, index, ty, span)| HirExpr::Subscript {
                        base: Box::new(base),
                        index: Box::new(index),
                        ty,
                        span,
                ...
                (
                    ...
                    .prop_map(|(expr, field, field_offset, ty, span)| {
                        HirExpr::FieldAccess {
                            expr: Box::new(expr),
                            field,
                            field_offset,
                ...
                (
                    ...
                    .prop_map(|(cond, then_expr, else_expr, ty, span)| {
                        HirExpr::Ternary {
                            cond: Box::new(cond),
...
fn arb_hir_stmt_leaf() -> impl Strategy<Value = HirStmt> {
    prop_oneof![
        ...
        (
            arb_var_id(),
            prop::option::of(arb_hir_expr_leaf()),
            arb_span()
        )
        ...
        // Expr
        (arb_hir_expr_leaf(), arb_span()).prop_map(|(expr, span)| HirStmt::Expr { expr, span }),
        // Return
        ...
        // Goto
        ("[a-z_]{1,8}", arb_span()).prop_map(|(label, span)| HirStmt::Goto { label, span }),
        // Label
        ("[a-z_]{1,8}", arb_span()).prop_map(|(name, span)| HirStmt::Label { name, span }),
        // Asm
        ("[a-z0-9 ]{1,16}", arb_span()).prop_map(|(text, span)| HirStmt::Asm { text, span }),
        // Comment
        ("[a-zA-Z0-9 ]{1,32}", arb_span()).prop_map(|(text, span)| HirStmt::Comment { text, span }),
    ]
}
        ...
        |inner| {
            prop_oneof![
                ...
                (
                    ...
                    .prop_map(|(cond, body, span)| HirStmt::While {
                        cond,
                        body,
                        span
                    }),
                ...
                (
                    ...
                    .prop_map(|(body, cond, span)| HirStmt::DoWhile {
                        body,
                        cond,
                        span
                    }),
                ...
                (
                    ...
                    prop::collection::vec(
                        (
                            ...
                            .prop_map(|(values, body, span)| HirCase {
                                values,
                                body,
                                span
                            }),
...
pub fn arb_lifted_module() -> impl Strategy<Value = LiftedModule> {
    (
        ...
        .prop_map(
            |(name, entry_address, arch, source_file, instructions)| LiftedModule {
                name,
                entry_address,
                arch: arch.to_string(),
                source_file,
