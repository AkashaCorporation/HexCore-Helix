...
fn benchmark_cfg(data: &CfgData, iterations: usize) {
    ...
    println!(
        "│  FlatBuffer: {} bytes, {:.3} ms ({} iters)",
        fb_size,
        fb_time.as_secs_f64() * 1000.0,
        iterations
    );
    println!(
        "│  JSON:       {} bytes, {:.3} ms ({} iters)",
        json_size,
        json_time.as_secs_f64() * 1000.0,
        iterations
    );
    println!(
        "│  Size ratio: {:.1}x smaller (FB vs JSON)",
        json_size as f64 / fb_size as f64
    );
    println!(
        "│  Speed ratio: {:.1}x faster (FB vs JSON)",
        json_time.as_secs_f64() / fb_time.as_secs_f64()
    );
    println!("└───────────────────────────────────────────────────────┘");
}
...
fn generate_report(cfg_data: &CfgData, ast_data: &AstData, iterations: usize) {
    ...
    let report = format!(
        ...
        cfg_fb.len(),
        cfg_json.len(),
        cfg_json.len() as f64 / cfg_fb.len() as f64,
        ast_fb.len(),
        ast_json.len(),
        ast_json.len() as f64 / ast_fb.len() as f64,
    );
