...
fn main() {
    ...
    for func in &hir.functions {
        ...
        println!("  Locals: {:?}", func.locals.iter().map(|l| format!("{} (id={})", l.name, l.id.0)).collect::<Vec<_>>());
    ...
    println!("  calls={}, args={}, eliminated={}", cc.calls_recovered, cc.args_recovered, cc.stmts_eliminated);

    let tp = type_propagation::propagate_types(&mut hir, &signatures);
    println!("━━━ After type propagation ━━━");
    println!("  resolved={}, unresolved={}", tp.types_resolved, tp.types_unresolved);
    ...
    for func in &hir.functions {
        ...
        println!("  Params: {:?}", func.params.iter().map(|p| format!("{} (id={})", p.name, p.id.0)).collect::<Vec<_>>());
        println!("  Locals: {:?}", func.locals.iter().map(|l| format!("{} (id={})", l.name, l.id.0)).collect::<Vec<_>>());
        println!("  Body ({} stmts):", func.body.len());
        for (i, stmt) in func.body.iter().enumerate() {
            println!("    [{}] {:?}", i, stmt);
